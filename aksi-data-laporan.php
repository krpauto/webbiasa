...
if (isset($_POST['simpanLaporanBaru'])) {
    ...
    if ($ukuran_file > 2097152) {
        ...
        header("location: page-data-laporan-baru.php");
    } else {
        ...
        if ($upload_file != "") {
            ...
            if (in_array($ekstensi, $ekstensi_file) === true) {
                ...
                // Query 
                $queryInsert = mysqli_query($conn, "INSERT INTO laporan VALUES('$id_laporan','$id_user','$judul','$tanggal_upload','$upload_file_new','$komentar','$status')");

                if ($queryInsert) {
                    $_SESSION['message'] = "File Laporan Berhasil Ditambahkan!";
                    $_SESSION['message_code'] = "success";
                    header("location: page-data-laporan-baru.php");
                } else {
                    $_SESSION['message'] = "File Laporan Gagal Ditambahkan!";
                    $_SESSION['message_code'] = "error";
                    header("location: page-data-laporan-baru.php");
                }
            } else {
            ...
            $queryInsert = mysqli_query($conn, "INSERT INTO laporan VALUES('$id_laporan','$id_user','$judul','$tanggal_upload')");
...
if (isset($_POST['updateLaporanBaru'])) {
    ...
    if ($ukuran_file > 1044070) {
        ...
        if ($upload_file != "") {
            ...
            if (in_array($ekstensi, $ekstensi_file) === true) {
                ...
                $get_pdf = "SELECT file FROM laporan WHERE id_laporan='$id_laporan'";
                ...
                // Query update
                $queryUpdate = mysqli_query($conn, "UPDATE laporan SET judul='$judul', tanggal_upload='$tanggal_upload', file='$upload_file_new' WHERE id_laporan='$id_laporan'");

                // mengecek apakah data gagal di input atau tidak
                if ($queryUpdate) {
                    $_SESSION['message'] = "File laporan Berhasil Diupdate!";
                    $_SESSION['message_code'] = "success";
                    header("location:page-data-laporan-baru.php");
                } else {
            ...
            // Query update
            $queryUpdate = mysqli_query($conn, "UPDATE laporan SET judul='$judul', tanggal_upload='$tanggal_upload' WHERE id_laporan='$id_laporan'");
...
if (isset($_POST['delete_btn_set'])) {
    ...
    $get_pdf    = "SELECT file FROM laporan WHERE id_laporan='$del_id'";
    $data_pdf   = mysqli_query($conn, $get_pdf);
    // Mengubah data yang diambil menjadi array
    $pdf_lama   = mysqli_fetch_array($data_pdf);
    ...
    // Query delete
    $queryDelete = mysqli_query($conn, "DELETE FROM laporan WHERE id_laporan='$del_id'");
}
